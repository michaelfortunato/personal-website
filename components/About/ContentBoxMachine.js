...
function generateSmoothLine(
  ...
  const spacing = 0.15; // spacing !== null ? spacing : 1/(ptsBetween + 1)
  const deltaX = Math.abs(xn - x0);
  const deltaY = yn - y0;

  const xDirection = xn - x0 < 0 ? -1 : 1;
  const prevYDirection = [];
  const pts = [];
  ...
  new Int8Array(ptsBetween + 1).forEach((val, i) => {
    ...
    if (i === 0) {
      ...
      const xi = pts[i - 1][0] + xDirection * (1 / (ptsBetween + 1)) * deltaX;
      const yScale =
...
const controlPoint =
  (smoothing) => (currentAnchor, previousAnchor, nextAnchor, direction) => {
    const previous = previousAnchor || currentAnchor;
    const next = nextAnchor || currentAnchor;
...
const SVGLine = (props) => {
  ...
  return mountable && <motion.path ref={props.lineRef} className="st6" d={d} />;
};
...
const SVGComponent = (props) => {
  ...
  const className = `${props.boxId.substring(
    0,
    props.boxId.indexOf("-"),
  )}-Circle`;
  return (
